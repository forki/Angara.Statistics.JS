 ...
 Runtime.Define(Global,{
  ...
  Statistics:{
   Complex:Runtime.Class({
    ...
    ImgExp:function(x)
    {
     return Complex.New(Math.cos(x),Math.sin(x));
    },
    New:function(real,imaginary)
    {
     var r;
     r=Runtime.New(this,{});
     r.re=real;
    ...
    {
     return Complex.New(x.get_Real()+y.get_Real(),x.get_Imaginary()+y.get_Imaginary());
    },
    op_Multiply:function(x,y)
    {
     return Complex.New(x.get_Real()*y.get_Real()-x.get_Imaginary()*y.get_Imaginary(),x.get_Imaginary()*y.get_Real()+x.get_Real()*y.get_Imaginary());
    },
    op_Subtraction:function(x,y)
    {
     return Complex.New(x.get_Real()-y.get_Real(),x.get_Imaginary()-y.get_Imaginary());
    }
   }),
   Radix2:function(samples,exponentSign)
   {
    var is_power_two,value,levelSize,k;
    is_power_two=function(x,p)
    {
     return x===p?true:x<p?false:is_power_two(x,2*p);
    };
    value=is_power_two(Arrays.length(samples),1);
    !value?Operators.FailWith("samples: The array length must be a power of 2."):null;
    Statistics.Radix2Reorder(samples);
    levelSize=1;
    while(levelSize<Arrays.length(samples))
     {
      for(k=0;k<=levelSize-1;k++){
       Statistics.Radix2Step(samples,exponentSign,levelSize,k);
      }
      levelSize=levelSize*2;
     }
    return;
   },
   Radix2Reorder:function(samples)
   {
    var j,i,_,temp,m,cont;
    j=0;
    for(i=0;i<=Arrays.length(samples)-2;i++){
     if(i<j)
      {
       temp=Arrays.get(samples,i);
       Arrays.set(samples,i,Arrays.get(samples,j));
       _=Arrays.set(samples,j,temp);
      }
     else
      {
       _=null;
      }
     m=Arrays.length(samples);
     cont=true;
     while(cont)
      {
       m=m>>1;
   ...
   Radix2Step:function(samples,exponentSign,levelSize,k)
   {
    var exponent,w,step,inputSequence,enumerator,_,i,ai,t;
    exponent=Number(exponentSign)*Number(k)*Statistics.pi()/Number(levelSize);
    w=Complex.New(Math.cos(exponent),Math.sin(exponent));
    step=levelSize<<1;
    inputSequence=Operators.step(k,step,Arrays.length(samples)-1);
    ...
    try
    {
     while(enumerator.MoveNext())
      {
       i=enumerator.get_Current();
       ai=Arrays.get(samples,i);
       t=Complex.op_Multiply(w,Arrays.get(samples,i+levelSize));
       Arrays.set(samples,i,Complex.op_Addition(ai,t));
       Arrays.set(samples,i+levelSize,Complex.op_Subtraction(ai,t));
      }
    }
    finally
    {
   ...
   {
    var x1,predicate,source,filtered,n,_,patternInput,_y,_x,patternInput1,sy,sx,patternInput2,stdy,stdx,_1,d1,d2,mapping,array;
    Arrays.length(x)!==Arrays.length(y)?Operators.FailWith("Different lengths of arrays"):null;
    x1=Seq.zip(x,y);
    predicate=function(tupledArg)
    {
     var u,v;
     u=tupledArg[0];
     v=tupledArg[1];
     return!(((Statistics.isNan(u)?true:Statistics.isNan(v))?true:Statistics.isInf(u))?true:Statistics.isInf(v));
    };
    source=Seq.filter(predicate,x1);
    filtered=Arrays.ofSeq(source);
    n=Arrays.length(filtered);
    if(n<=1)
     {
      _=NaN1;
     }
     ...
     {
      patternInput=[Arrays.map(function(tuple)
      {
       return tuple[0];
      },filtered),Arrays.map(function(tuple)
      {
       return tuple[1];
      },filtered)];
      _y=patternInput[1];
      _x=patternInput[0];
      patternInput1=[Statistics.summary(_x),Statistics.summary(_y)];
      sy=patternInput1[1];
      sx=patternInput1[0];
      patternInput2=[Math.sqrt(sx.variance),Math.sqrt(sy.variance)];
      stdy=patternInput2[1];
      stdx=patternInput2[0];
      if(((stdx===0?true:stdy===0)?true:Statistics.isInf(stdx))?true:Statistics.isInf(stdy))
       {
        _1=NaN1;
       }
      else
       {
        d1=Number(n)*sx.mean*sy.mean;
        d2=Number(n-1)*stdx*stdy;
        mapping=function(tupledArg)
        {
         var s,t;
         s=tupledArg[0];
        ...
        array=Arrays.map(mapping,filtered);
        _1=(Arrays.sum(array)-d1)/d2;
       }
      _=_1;
     }
    return _;
   ...
   dct:function(rxs)
   {
    var xs,len,n,myseq,source,weights,en,en2,ind,mapping,source1,interleaved;
    xs=Arrays.map(function(x)
    {
     return Complex.New(x,0);
    },rxs);
    len=Arrays.length(xs);
    n=Statistics.fi(len);
    myseq=Seq.init(len-1,function(x)
    {
     return Complex.op_Multiply(Complex.New(2,0),Complex.ImgExp(Statistics.fi(x+1)*Statistics.pi()/(2*n)));
    });
    source=Seq.delay(function()
    {
     return Seq.append([Complex.New(2,0)],Seq.delay(function()
     {
      return myseq;
     }));
    });
    weights=Arrays.ofSeq(source);
    en=Seq.init((len+1)/2>>0,function(i)
    {
     return i*2;
    });
    en2=Seq.init(len/2>>0,function(i)
    {
     return len-i*2-1;
    });
    ind=Seq.delay(function()
    {
     return Seq.append(en,Seq.delay(function()
     {
      return en2;
     }));
    });
    mapping=function(i)
    {
     return Arrays.get(xs,i);
    };
    source1=Seq.map(mapping,ind);
    interleaved=Arrays.ofSeq(source1);
    return Arrays.map2(function(a)
    {
     return function(b)
     {
      return Complex.op_Multiply(a,b).get_Real();
     };
    },weights,Statistics.fft(interleaved));
   },
   e:Runtime.Field(function()
   {
   ...
   histogram_:function(n,xmin,xmax,xs)
   {
    var isNotFinite,h,step,add;
    n<1?Operators.FailWith("n: must be > 0"):null;
    isNotFinite=function(x)
    {
     return isNaN(x)?true:Math.abs(x)===Infinity1;
    };
    isNotFinite(xmin)?Operators.FailWith("xmin: is "+xmin.toExponential()):null;
    isNotFinite(xmax)?Operators.FailWith("xmax: is "+xmax.toExponential()):null;
    xmin>=xmax?Operators.FailWith("xmin should be less than xmax"):null;
    h=Array(n);
    Arrays.iteri(function(i)
    {
     return function()
     {
      return Arrays.set(h,i,0);
     };
    },h);
    step=(xmax-xmin)/Number(n);
    add=function(x)
    {
     var _,idx;
     if((!Statistics.isNan(x)?x>=xmin:false)?x<=xmax:false)
      {
       idx=Operators.Min(n-1,(x-xmin)/step<<0);
       _=Arrays.set(h,idx,Arrays.get(h,idx)+1);
      }
     else
      {
       _=null;
   ...
   idct:function(rxs)
   {
    var xs,len,n,coeff,weights,mapping,x1,array,vals,interleave;
    xs=Arrays.map(function(x)
    {
     return Complex.New(x,0);
    },rxs);
    len=Arrays.length(rxs);
    n=Statistics.fi(len);
    coeff=function(k)
    {
     return Complex.op_Multiply(Complex.New(2*n,0),Complex.ImgExp(Statistics.fi(k+1)*Statistics.pi()/(2*n)));
    };
    weights=Seq.toArray(Seq.delay(function()
    {
     return Seq.append([Complex.New(n,0)],Seq.delay(function()
     {
      return Seq.map(function(i)
      {
       return coeff(i);
    ...
    x1=Arrays.map2(function(x)
    {
     return function(y)
     {
      return Complex.op_Multiply(x,y);
     };
    },weights,xs);
    array=Statistics.ifft(x1);
    vals=Arrays.map(mapping,array);
    interleave=function(z)
    {
     var hz,activePatternResult;
     hz=z>>1;
     activePatternResult=Statistics["|Even|Odd|"](z);
     return activePatternResult.$==1?Arrays.get(vals,len-hz-1):Arrays.get(vals,hz);
    };
    return Seq.toArray(Seq.delay(function()
    {
     return Seq.map(function(i)
   ...
   ifft:function(xs)
   {
    var samples,scalingFactor,i,v;
    samples=xs.slice();
    Statistics.Radix2(samples,1);
    scalingFactor=1/Number(Arrays.length(samples));
    for(i=0;i<=Arrays.length(samples)-1;i++){
     v=Arrays.get(samples,i);
     Arrays.set(samples,i,Complex.New(scalingFactor*v.get_Real(),scalingFactor*v.get_Imaginary()));
    }
    return samples;
   },
   improbable:Runtime.Field(function()
   {
    return 2.2250738585072E-308;
   }),
   isInf:function(arg00)
   {
    return Math.abs(arg00)===Infinity1;
   },
   isNan:function(arg00)
   {
    return isNaN(arg00);
   },
   kde:function(n0,xs)
   {
    var _,max,min,range,_1,action;
    if(Unchecked.Equals(xs,null))
     {
      _=Operators.FailWith("sample: cannot be empty");
     }
    else
     {
      ...
      if(xs.length==0)
      ...
      else
       {
        action=function(xsi)
        {
         max[0]<xsi?void(max[0]=xsi):null;
         return min[0]>xsi?void(min[0]=xsi):null;
        };
        Arrays.iter(action,xs);
        _1=min[0]>=max[0]?1:max[0]-min[0];
       }
      range=_1;
      _=Statistics.kde2(n0,min[0]-range/10,max[0]+range/10,xs);
     }
    return _;
   },
   kde2:function(n0,min,max,sample)
   {
    var _,_1,xs,m_sqrt_2_pi,m_sqrt_pi,r,len,ni,n,sqr,d,mesh,h,sh,mapping,rxs,a1,iv,mapping1,array,a2v,t_star,f,go,eq,matchValue,_2,root,f2,a21,a11,a0;
    if(Unchecked.Equals(sample,null))
     {
      _=Operators.FailWith("sample cannot be null");
     }
    else
     {
      if(n0===1)
       {
        _1=Operators.FailWith("n0 cannot be 1");
       }
      else
       {
        xs=Arrays.filter(function(x)
        {
         var value;
         value=isNaN(x);
         return!value;
        },sample);
        xs.length==0?Operators.FailWith("sample: doesn't contain numeric values"):null;
        m_sqrt_2_pi=Math.sqrt(2*Statistics.pi());
        m_sqrt_pi=Math.sqrt(Statistics.pi());
        r=max-min;
        len=Statistics.fi(Arrays.length(xs));
        ni=Statistics.nextHighestPowerOfTwo(n0);
        n=Statistics.fi(ni);
        sqr=function(a)
        {
         return a*a;
        ...
        d=r/(n-1);
        mesh=Arrays.init(ni,function(z)
        {
         return min+d*Statistics.fi(z);
        });
        h=Arrays.map(function(x)
        {
         return Number(x)/len;
        },Statistics.histogram_(ni,min,max,xs));
        sh=Arrays.sum(h);
        mapping=function(p)
        {
         return p/sh;
        };
        rxs=Arrays.map(mapping,h);
        a1=Statistics.dct(rxs);
        iv=Seq.toArray(Seq.delay(function()
        {
         return Seq.map(function(i)
         {
          return sqr(Statistics.fi(i));
         },Operators.range(1,ni-1));
        }));
        mapping1=function(q)
        {
         return sqr(q*0.5);
        };
        array=Arrays.skip(1,a1);
        a2v=Arrays.map(mapping1,array);
        f=function(q,t)
        {
         var g;
         g=function(i)
         {
          return function(a2)
          {
           return Math.pow(i,q)*a2*Math.exp(-i*sqr(Statistics.pi())*t);
          };
         };
         return 2*Math.pow(Statistics.pi(),q*2)*Arrays.sum(Arrays.map2(g,iv,a2v));
        };
        go=function(s,h1)
        {
         var si,_enum,k0,_const,time;
         si=Statistics.fi(s);
         _enum=Seq.delay(function()
         {
          return Seq.map(function(i)
          {
           return 2*i-1;
         ...
         k0=Statistics.fi(Seq.fold(function(x)
         {
          return function(y)
          {
           return x*y;
         ...
         _const=(1+Math.pow(0.5,si+0.5))/3;
         time=Math.pow(2*_const*k0/len/h1,2/(3+2*si));
         return s===1?h1:go(s-1,f(si,time));
        };
        eq=function(x)
        {
         return x-Math.pow(len*(2*m_sqrt_pi)*go(6,f(7,x)),-0.4);
        };
        matchValue=Statistics.ridders(1E-14,0,0.1,eq);
        if(matchValue.$==0)
         {
          _2=0.28*Math.pow(len,-0.4);
         }
        else
         {
          root=matchValue.$0;
        ...
        f2=function(b)
        {
         return function(z)
         {
          return b*Math.exp(sqr(z)*sqr(Statistics.pi())*t_star*-0.5);
         };
        };
        a21=Arrays.map2(f2,a1,Seq.toArray(Seq.delay(function()
        {
         return Seq.map(function(i)
         {
          return Statistics.fi(i);
         },Operators.range(0,ni-1));
        })));
        a11=Statistics.idct(a21);
        a0=Arrays.map(function(x)
        {
         return x/(2*r);
        },a11);
        _1=[mesh,a0];
   ...
   log_improbable:Runtime.Field(function()
   {
    return Math.log(Statistics.improbable());
   }),
   log_tolerance:Runtime.Field(function()
   {
    return Math.log(Statistics.tolerance());
   }),
   nextHighestPowerOfTwo:function(n)
   {
    var i0,i1,i2,i4,i8,i16,_i32;
    i0=n-1;
    i1=i0|i0>>1;
    i2=i1|i1>>2;
    i4=i2|i2>>4;
   ...
   pi:Runtime.Field(function()
   {
    return 3.14159265358979;
   }),
   pi2:Runtime.Field(function()
   {
    return 6.28318530717959;
   }),
   qsummary:function(data)
   {
    var predicate,source,a,n,_,q;
    predicate=function(d)
    {
     return!(isNaN(d)?true:Math.abs(d)===Infinity1);
    };
    source=Seq.filter(predicate,data);
    a=Seq.toArray(source);
    Arrays.sortInPlace(a);
    n=Arrays.length(a);
    if(n<1)
     {
    ...
    else
     {
       ...
       if(h<=0)
       ...
       else
        {
         if(h>=Number(n-1))
          ...
          {
           fh=Math.floor(h);
           _2=Arrays.get(a,fh<<0)*(1-h+fh)+Arrays.get(a,(fh<<0)+1)*(h-fh);
          }
         _1=_2;
        }
       return _1;
      ...
      _={
       ...
       ub68:q(0.84),
       ub95:q(0.975),
       max:Arrays.get(a,n-1)
      };
     }
    return _;
   },
   ridders:function(tolerance,lb,ub,f)
   {
    var loop,flb,_,fub,_1,_2,_3,_4,d,dm,m,fm,_5,dn,n,_6,_7,_8,_9,_a,_b,_c,fn,_d,_e,_f,_10,_11,_12,_13;
    loop=[];
    !(tolerance>=0)?Operators.FailWith("tolerance must be greater than 0.0"):null;
    flb=f(lb);
    if(0===flb)
     {
      loop[0]=0;
    ...
    while(loop[0])
     {
      if(100<=loop[5])
       ...
       {
        if(Statistics.within(1,loop[1],loop[3]))
         {
          loop[0]=0;
          _4=void(loop[1]={
           $:1,
         ...
         {
          d=Math.abs(loop[3]-loop[1]);
          dm=(loop[3]-loop[1])*0.5;
          m=loop[1]+dm;
          fm=f(m);
          if(0===fm)
           {
           ...
           {
            dn=Number(Operators.Sign(loop[4]-loop[2]))*dm*fm/Math.sqrt(fm*fm-loop[2]*loop[4]);
            n=m-Number(Operators.Sign(dn))*Operators.Min(Math.abs(dn),Math.abs(dm)-0.5*tolerance);
            if(d<tolerance)
             {
              loop[0]=0;
              _6=void(loop[1]={
             ...
             {
              if(n===loop[1]?true:n===loop[3])
               {
                if(0>Operators.Sign(fm)*Operators.Sign(loop[2]))
                 {
                  _9=loop[2];
                  _a=loop[1];
                  loop[5]=loop[5]+1;
   ...
   {
    var folder,pass,_1,variance,pass1;
    folder=function(summary)
    {
     return function(d)
     {
      var _,delta,n,mean;
      if(Statistics.isNan(d)?true:Statistics.isInf(d))
       {
        _=summary;
       }
      else
       {
        delta=d-summary.mean;
        n=summary.count+1;
        mean=summary.mean+delta/Number(n);
        _={
         count:n,
         min:Operators.Min(d,summary.min),
         max:Operators.Max(d,summary.max),
         mean:mean,
    ...
    if(pass.count<2)
     ...
     {
      variance=pass.variance/Number(pass.count-1);
      pass1={
       count:pass.count,
       min:pass.min,
       max:pass.max,
   ...
   tolerance:Runtime.Field(function()
   {
    return 1.11022302462516E-16;
   }),
   within:function(ulps,a,b)
   {
    return Math.abs(a-b)<Statistics.tolerance();
   },
   "|Even|Odd|":function(input)
   {
    return input%2===0?{
     $:0,
     $0:null
  ...
  StatisticsHelpers:{
   getTableViewerSource:function(columnNames,columnData)
   {
    ...
    summaries=Seq.toArray(Seq.delay(function()
    {
     return Seq.map(function(i)
     {
      return Statistics.summary(Arrays.get(columnData,i));
     },Operators.range(0,len-1));
    }));
    qsummaries=Seq.toArray(Seq.delay(function()
    {
     return Seq.map(function(i)
     {
    ...
    merger=function(i)
    {
     return function(sum)
     {
      return function(qsum)
      {
       return{
        name:Arrays.get(columnNames,i),
        type:"numeric",
        count:sum.count,
